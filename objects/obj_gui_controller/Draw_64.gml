if instance_exists(obj_character) and global.t != 0 {
if obj_character.reloading {
	// рисуем снизу вверх, это точка на которой мы рисуем сейчас
	// после каждого отрисованного спрайта мы прибавляем к ней v_align + высоту спрайта * gui_scale
	var _drawing_point = v_align
	
	#region рисуем барабан
	
	var _drum_x = screen_width - h_align - 32 * global.gui_scale
	
	draw_sprite_ext(
		sBaraban, 0, 
		_drum_x, 
		_drawing_point, 
		global.gui_scale, global.gui_scale, 1, 
		c_white, 1
	)

	// рисуем пули в барабане
	for (var _i=0; _i < array_length(obj_character.bullets); _i++) {
		draw_sprite_ext(
			obj_character.bullets[_i], 0, 
			_drum_x + bullets_cords[_i][0] * global.gui_scale, 
			_drawing_point + bullets_cords[_i][1] * global.gui_scale, 
			global.gui_scale, global.gui_scale, 1, 
			c_white, 1
		)
	}
	
	#endregion
	
	_drawing_point += v_align + sprite_get_height(sBaraban) * global.gui_scale
	
	#region рисуем пули
	
	var _bullets_x = screen_width - h_align - 24 * global.gui_scale
	
	for (var _i=0; _i < array_length(unlocked_bullets); _i++) {
		if !instance_exists(unlocked_bullets[_i]) {
			instances[_i] = instance_create_layer(
				_bullets_x,
				_drawing_point,
				"gui_instances",
				unlocked_bullets[_i]
			)
			
			_drawing_point += v_align + sprite_get_height(sBullet_Choice) * global.gui_scale
		}
	}
	
	#endregion
} else array_foreach(instances, instance_destroy)
}
draw_self()